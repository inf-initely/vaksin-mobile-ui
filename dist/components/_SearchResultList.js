...
import {HStack, Link, Text} from "../../_snowpack/pkg/@chakra-ui/react.js";
const SearchResultList = (props) => {
  const {fuseResult, onClickResult, InputElement, ...rest} = props;
  const [isFocused, setFocused] = useState(document.activeElement === InputElement.current);
  ...
  useEffect(() => {
    if (RegionList.current == null)
      return;
    if (isFocused)
      RegionList.current.hidden = false;
  }, [isFocused]);
  document.onclick = (e) => {
    if (isFocused)
      return;
  ...
  return /* @__PURE__ */ React.createElement(HStack, {
    ...
    return /* @__PURE__ */ React.createElement(Link, {
      ...
      onClick: () => {
        ...
        RegionList.current.hidden = true;
        InputElement.current.value = result.item.city.replace("Kota ", "").replace("Kab. ", "");
      },
      py: 1,
      pl: 3
